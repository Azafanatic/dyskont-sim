...
int main(int argc, char *argv[]) {
    ...
    int shm_id = shmget(IPC_PRIVATE, sizeof(Semafory), IPC_CREAT | 0666);
    if (shm_id == -1) {
        perror("shmget");
        exit(1);
    }

    Semafory *shared = (Semafory *)shmat(shm_id, NULL, 0);
    if (shared == (void *)-1) {
        perror("shmat");
        exit(1);
    }
    ...
    if (pids[1] < 0) {
        ...
        execlp("./kierownik", "kierownik", shm_id_str,(char *)NULL);
        zapisz_wiadomosc(COL_RED, "Błąd exec dla kierownik\n");
        exit(1);
    }
    ...
    pids[2] = fork();
    if (pids[2] < 0) {
        zapisz_wiadomosc(COL_RED, "Błąd forka klient\n");
        exit(1);
    } else if (pids[2] == 0) {
        execlp("./klient", "klient", shm_id_str, (char *)NULL);
        zapisz_wiadomosc(COL_RED, "Błąd exec dla klient\n");
        exit(1);
    }
